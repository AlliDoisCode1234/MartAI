...
export async function generateDraftFromBrief(
  ...
  const primaryTitle = brief.titleOptions && brief.titleOptions.length > 0 
...
function calculateToneScore(content: string, brandVoice?: string): number {
  ...
  const engagingWords = (content.match(/\b(you|your|we|our|let's|discover|learn|explore)\b/gi) || []).length;
 ...
 */
export function validateDraftSEO(draft: DraftResult, brief: BriefInfo): {
  valid: boolean;
  ...
  const checklist = [
    ...
    {
      item: 'FAQs addressed',
      passed: brief.faqs ? brief.faqs.every(faq => 
