...
${siteAnalysis ? `Site Analysis Results:
...
Perform a comprehensive SEO audit and provide actionable recommendations based on 2025-2026 SEO best practices, Google's latest SEO guidelines, and social media platform SEO strategies.`;
    ...
    const result = await generateText({
      ...
      tools: {
        seoAudit: tool({
          description: 'Perform a comprehensive SEO audit analysis for a business website. Analyze technical SEO, on-page optimization, content quality, backlinks, and provide actionable recommendations.',
          ...
          execute: async ({ website, industry, companyName, siteAnalysis }) => {
            ...
            const onPageScore = siteAnalysis 
              ? Math.max(0, 100 - (issues.filter((i: string) => i.includes('H1') || i.includes('meta') || i.includes('heading')).length * 15))
              : 68;
            ...
            return {
              technicalSeo: {
                ...
                issues: siteAnalysis?.issues.filter((i: string) => 
                  i.includes('SSL') || i.includes('mobile') || i.includes('speed') || i.includes('canonical')
                ) || [
                ...
                recommendations: [
                  ...
                  siteAnalysis && !siteAnalysis.mobileFriendly ? 'Add viewport meta tag for mobile optimization' : '',
                ].filter(Boolean),
              },
              onPageSeo: {
                score: Math.max(0, Math.min(100, onPageScore)),
                issues: siteAnalysis?.issues.filter((i: string) => 
                  i.includes('H1') || i.includes('heading') || i.includes('title')
                ) || [
                ...
                recommendations: [
                  ...
                  siteAnalysis && siteAnalysis.h1Tags.length > 1 ? 'Reduce to single H1 per page' : '',
                ].filter(Boolean),
              },
              contentQuality: {
                score: Math.max(0, Math.min(100, contentScore)),
                issues: [
                  siteAnalysis && siteAnalysis.wordCount < 300 ? `Low word count (${siteAnalysis.wordCount} words, recommended 300+)` : 'Content freshness: last major update needed',
                ...
                recommendations: [
                  ...
                  siteAnalysis && siteAnalysis.wordCount < 300 ? 'Increase content depth with more detailed information' : '',
                ].filter(Boolean),
              },
              backlinks: {
                score: 65,
        ...
        generateKeywords: tool({
          ...
          execute: async ({ companyName, industry, targetAudience, website }) => {
            try {
              const keywords = await generateKeywords(companyName, industry, targetAudience, website);
              return keywords;
            } catch (error) {
              console.error('Keyword generation error:', error);
              // Fallback keywords
              return [
                { keyword: `${industry} services`, intent: 'commercial', priority: 'high', reasoning: 'Primary service keyword' },
                { keyword: `best ${industry}`, intent: 'commercial', priority: 'high', reasoning: 'High commercial intent' },
                { keyword: `${targetAudience} ${industry}`, intent: 'transactional', priority: 'high', reasoning: 'Target audience specific' },
              ];
            }
          },
        }),
        generateContent: tool({
          description: 'Generate SEO-optimized content suggestions including taglines, SEO-friendly headlines, meta descriptions, and content ideas.',
          inputSchema: z.object({
            companyName: z.string(),
            industry: z.string(),
            targetAudience: z.string(),
        ...
        generateSocialMediaPosts: tool({
          description: 'Generate SEO-optimized social media post suggestions for multiple platforms.',
          inputSchema: z.object({
            companyName: z.string(),
            industry: z.string(),
            targetAudience: z.string(),
